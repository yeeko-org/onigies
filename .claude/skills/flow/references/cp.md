...
cp is published to real IES since 2026-09-25 (`PUBLISHED_SECTIONS` in `nuxt/app/utils/sections.js`), with `Period.cp_open_at` set the same day; what still keeps a real IES from capturing is the `gen_finished` prerequisite of its general package. `/respuestas/[period]` also offers, on the axis tabs only, the public Word download of the questionnaire (`utils/public_documents.js`).
